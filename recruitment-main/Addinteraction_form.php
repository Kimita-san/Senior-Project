...
$Followup = $_POST['followupDate'];
$fDate = date("Y-m-d",strtotime($Followup));
$comments = $_POST['comment'];

 $sql = "INSERT INTO Interactions (studentID, studentName,userID, activity, comments, date, followupDate)
          VALUES ('$idstu', '$c', '$userID', '$activity', '$comments', '$cDate', '$fDate')";
          if (mysqli_query($conn, $sql)) {
             echo "New Interaction has been added successfully !";
           } else {
              echo "Error: " . $sql . ":-" . mysqli_error($conn);
           }
  $sql2 = "UPDATE students SET followupDate = '$fDate' WHERE studentID = $id";
   if (mysqli_query($conn, $sql2)) {
     echo "<script> alert('Update Successful'); </script>";
     header("location: profile.php?display=$idstu");
   }
